...
export default defineContentScript({
  ...
  async main() {
    const inputSelector = "div[contenteditable='true']"; // Selector for the input element
    let aiIcon: HTMLDivElement | null = null;

    // Create and configure the AI icon
    function createAIIcon(targetElement: HTMLElement) {
      const parentDiv = targetElement.parentElement;
      if (!parentDiv) return; // Exit if the parent is not found

      aiIcon = document.createElement("div");
      aiIcon.id = "ai-icon";
      aiIcon.innerHTML = getSVGIcon(); // Insert SVG icon
      styleAIIcon(aiIcon);

      parentDiv.appendChild(aiIcon); // Append the icon to the parent
      aiIcon.addEventListener("click", openModal); // Add click listener
    }

    // Get SVG icon markup
    function getSVGIcon() {
      return `<svg width="14" height="12" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    // Style the AI icon
    function styleAIIcon(icon: HTMLDivElement) {
      ...
      icon.style.border = "1px solid #e5e7eb";
      icon.style.padding = "0.5rem";
    }

    // Remove existing AI icon if it exists
    function removeExistingAIIcons() {
    ...
    // Open modal when the AI icon is clicked
    function openModal() {
      mountModal(insertResponse);
    }

    // Send response text to be inserted
    ...
    // Handle focus event to create AI icon
    function handleFocusIn(event: Event) {
      const target = event.target as HTMLElement;
      if (target.matches(inputSelector)) {
        if (!aiIcon) createAIIcon(target);
      }
    }

    // Handle focus out event to remove AI icon
    function handleFocusOut(event: FocusEvent) {
      const relatedTarget = event.relatedTarget as HTMLElement;
      if (!aiIcon || relatedTarget === aiIcon) return;

      removeExistingAIIcons();
    }
